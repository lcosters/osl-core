...
try, permmeth = S.permmeth; catch, permmeth = 'clustextent'; end
...
if strcmp(S.permmeth, 'clustextent')
    ...
    for r = 1:nP/100;
        ...
        for t = tp;
            ...
            fprintf(fid,['randomise -d ' [scriptdir '/design.mat'] ' -t ' [scriptdir '/design.con'] ' -i %s/allsubs_time%04.0f -o %s/stats%04.0f -c %1.2f -R -n 100 ' ...
                ...
                dirname,t, permdir,t,thresh,r,dirname,t); % -c means cluster-extent thresholding; -C means cluster-mass thresholding
        end
    end
    fclose(fid);
    ...
    for t = tp;
        permdir = sprintf('%s/%s/%04.0f',dirname,subdirname,t);
        for p = 1:100;
            fprintf(fid,['cluster -i %s/stats%04.0f_clustere_tstat1_perm%05.0f -t %1.2f -o %s/cindex%05.0f; '],...
                permdir,t,p,thresh,permdir,(r-1)*100+p);
        end
    end
    fclose(fid);
...
elseif strcmp(S.permmeth, 'clustmass') 
    ...
    %% write into overarching script
    if(~write_cluster_script),
        fprintf(fidM,'%s/split_merge_randomise5_1_1.sh  \n',scriptdir);
    else
        fprintf(fidM,'jid1=`fsl_sub -q veryshort.q -j $jid3 -t %s/split_merge_randomise5_%0.0f_1.sh -l %s` \n',scriptdir,r,lfname);
    end;
    ...
    distfile_save=sprintf('%s/%s/dist.mat',dirname,subdirname);
    mfile = sprintf('%s/c4ddist.m',scriptdir);
    fid = fopen(mfile,'w');
